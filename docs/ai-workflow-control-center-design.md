...
目前控制台已能查詢 skills、複製提示詞、查看二刀流工作流與角色導覽。下一步希望把它升級成更完整的「AI 協作中控台」：

- 遇到新的好用 skill 或提示詞時，能用表單收錄，產生 YAML 片段與給 Codex 的完整交辦提示詞。
- 二刀流工作流進行中，能用靜態教學與提示詞快速入口提醒使用者下一步該找誰。
...
- v1 不做 Codex thread 或 Claude Code（VS Code） 的自動送出。
...
專案啟動與二刀流工作流提示詞應支援「跨地點、跨系統接續」情境；v1 新增獨立提示詞卡 `跨地點 / 跨系統接續`。
...
把二刀流工作流從「手動轉貼」升級成更清楚的「透明中控」。v1 不做動態狀態或自動送出，只做靜態五階段教學與提示詞快速入口。
...
- 要交給誰：Codex / Claude Code（VS Code）
...
2. 雙擊 `index.html` 可看到新增 `收錄新內容` 與 `二刀流中控` 頁籤。
3. 既有六個頁籤功能與搜尋未被打破：`Skills`、`提示詞庫`、`二刀流工作流`、`AI 角色導覽`、`安檢 SOP`、`換電腦／同步`。
...
8. `二刀流中控` 只顯示靜態五階段教學與提示詞快速入口，不讀取 `DUAL-AI-STATE.md`。
9. `二刀流中控` 快速入口的按鈕語意是跳到提示詞庫對應 stage 並複製 prompt，不暗示自動送出。
...
- 才探索 Codex thread、Claude Code（VS Code） 的自動送出能力。

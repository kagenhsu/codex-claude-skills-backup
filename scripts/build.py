...
const PAGE_INTROS = {
  ...
  guide:{title:"AI 角色導覽",lead:"這頁幫新手判斷 Codex、Claude Code（VS Code）、Claude Desktop 各自適合做什麼，也說明要用哪個入口才會一起啟動配額守門員。",purpose:"快速判斷誰負責規劃、實作、審查、文件或提示詞，並用對啟動入口。",first:"只看控制台可開 index.html；要一起開配額守門員，請用「開啟控制台與配額守門員」啟動器。",when:"不確定要叫哪個 AI 處理，或同事只有單一 AI 可用、第一次開這套系統時。"},
...
function cats(){if(tab==="skills")return["全部",...new Set(DATA.skills.map(s=>s.category))];if(tab==="prompts")return["全部","專案啟動","日常工作"];return[]}
...
const PROMPT_CATEGORIES=["專案啟動","開發過程","版本控制","整合串接","上線部署","三方 AI 協作","Skill 管理","安全檢查","三方 AI 工作流","單AI精簡","日常工作","其他"];

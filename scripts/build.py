...
function customSkillArtifactCard(title,body,copyLabel){return`<div class="card"><div class="section-head"><h2>${esc(title)}</h2><span class="hint">${esc(copyLabel)}</span></div><pre class="prompt-body builder-preview">${esc(body)}</pre><div class="home-panel-actions"><button class="copy-btn" type="button" data-copy="${encodeURIComponent(body)}">${esc(copyLabel)}</button></div></div>`}

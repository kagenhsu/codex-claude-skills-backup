...
function bundledSkillsInstallHtml(){
  ...
  const langGptPrompt=`請幫我安裝 LangGPT，但要先安檢，安全才可以安裝。\n\n請依序處理：\n1. 先確認 LangGPT 的官方或可信來源，不要用來路不明的壓縮包。\n2. 安裝前先檢查：是否有外部下載腳本、是否會讀取或上傳憑證、是否有提示詞注入風險。\n3. 如果風險不可接受，就停止安裝，直接告訴我原因。\n4. 如果風險可接受，再安裝到這兩個位置：\n   - ~/.codex/skills/\n   - ~/.claude/skills/\n5. 安裝完成後，驗證 Codex 與 Claude Code 兩邊都能讀到 LangGPT。\n6. 最後用繁體中文回報：來源、安檢結論、安裝位置、驗證結果、還有沒有剩餘風險。`;
  return `<div class="card install-hero" style="margin-top:14px"><h2>📦 一鍵安裝這個 repo 內附的 skills</h2><div class="summary">這裡直接給你「一行安裝全部」的指令。按下複製後，貼到終端機或 PowerShell 按 Enter，就會把這個 repo 備份包內實際存在的 skill 一次裝到 <code>~/.codex/skills</code> 與 <code>~/.claude/skills</code>。</div>${cmdRow("macOS / Linux：一行安裝這個 repo 內全部 skills",installAllMac)}${cmdRow("Windows：一行安裝這個 repo 內全部 skills",installAllWin)}<div class="privacy-note">重要：目前這個 repo 備份包內實際只有 <code>dual-ai-workflow</code> 與 <code>plan-progress</code> 兩個真實 skill。網站上其他 baoyu-* 卡片是說明資料，不在這個 repo 備份包裡，不能用這裡一鍵安裝。</div><div class="summary" style="margin-top:12px"><b>建議一起裝的必備 skill：LangGPT</b></div><div class="summary" style="margin-top:6px">如果你後面會自己做 prompt、設計職業型 skill，或想把工作流程整理成可重複使用的 AI 模板，建議把 <b>LangGPT</b> 一起裝起來。它不在這個 repo 備份包裡，所以這裡不做一鍵腳本，改成提供一顆可複製的安裝提示詞按鈕。</div><div class="home-panel-actions" style="margin-top:10px"><button class="copy-btn" data-copy="${encodeURIComponent(langGptPrompt)}">複製 LangGPT 安裝提示詞</button></div></div>`;
}

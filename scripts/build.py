...
let tab = "guide", cat = "全部", q = "", flowMode = "dualai", captureMode = "prompt", progressMode = "status", selectedProject = null;
const FLOW_META = {
...
const PROJECT_STAGES = [
  ["v1.0","控制台初版","Skill 目錄、提示詞庫、搜尋與安檢 SOP。"],
  ["v1.1","快速看板與組合包","三方中控快速看板與常用流程一包複製。"],
...
function progressSource(){return selectedProject||{name:"目前控制台資料夾",path:PROJECT_PATH,state:STATE_HTML,next:NEXT_HTML,agents:AGENTS_HTML,prd:PRD_HTML}}
...
function progressPickerHtml(src){return`<div class="warning"><div>請選擇要查看的專案資料夾；系統會自動檢查 DUAL-AI-STATE.md / NEXT-AI-TASK.md / AGENTS.md / PRD.md 是否存在。</div><div class="trigger" style="margin-top:8px"><code>${esc(src.path||src.name)}</code><label class="copy-btn" for="projectFolderInput">選擇專案資料夾</label><button class="copy-btn" data-copy="${encodeURIComponent(src.path||src.name)}">複製目前名稱</button><input id="projectFolderInput" type="file" webkitdirectory directory multiple style="display:none"></div></div>`}
...
function progressStageMapHtml(parsed){const currentVersion=(parsed.current.match(/v\d+\.\d+/)||[])[0]||"";return`<div class="card"><h2>專案版本地圖</h2><div class="summary">這裡顯示整個控制台從 v1.0 到目前版本的開發階段；目前版本會用藍框標出來。</div><div class="workflow-guide">${PROJECT_STAGES.map(([version,title,desc])=>{const active=currentVersion===version?" state-stage-active":"";return`<div class="guide-item${active}"><div class="guide-label">${esc(version)}｜${esc(title)}</div><div class="guide-text">${esc(desc)}</div></div>`}).join("")}</div></div>`}
function progressStatusHtml(parsed,src){const summary=parsed.ok?`<div class="state-summary"><div class="state-item full"><div class="state-label">目前階段</div><div class="state-value">${esc(parsed.current)}</div></div><div class="state-item full"><div class="state-label">上一步做了什麼</div><div class="state-value">${esc(parsed.done)}</div></div><div class="state-item full"><div class="state-label">下一步</div><div class="state-value">${esc(parsed.next)}</div></div><div class="state-item full"><div class="state-label">未解決問題</div><div class="state-value">${esc(parsed.unresolved)}</div></div></div>`:stateSummaryHtml(parsed);return`${progressStageMapHtml(parsed)}${summary}`}
...
function progressStagesHtml(src){return`<div class="card"><h2>三方 AI 開發階段</h2><div class="workflow-guide">${FLOW_ORDER.dualai.filter(stage=>["1","2","3","4","5"].includes(stage)).map(stage=>{const meta=STAGE_META.dualai[stage];return`<div class="guide-item"><div class="guide-label">${esc(meta[0])}</div><div class="guide-text">${esc(meta[1])}</div></div>`}).join("")}</div></div><div class="card"><h2>本專案 / 網站開發階段</h2><div class="summary">${esc(mdSection(src.prd.raw,"1. 專案背景"))}</div><div class="summary">${esc(mdSection(src.prd.raw,"2. 專案目標"))}</div><div class="summary">${esc(mdSection(src.prd.raw,"6. 驗收標準"))}</div></div>`}
...
async function loadProjectFolder(files){const list=[...files];const root=(list[0]?.webkitRelativePath||"").split("/")[0]||"使用者選擇的專案";const find=name=>list.find(file=>file.name===name||file.webkitRelativePath.endsWith(`/${name}`));const read=async name=>{const file=find(name);return file?await file.text():""};const state=await read("DUAL-AI-STATE.md"),next=await read("NEXT-AI-TASK.md"),agents=await read("AGENTS.md"),prd=await read("PRD.md");selectedProject={name:root,path:root,state:{raw:state,html:simpleMarkdownHtml(state,"DUAL-AI-STATE")},next:{raw:next,html:simpleMarkdownHtml(next,"NEXT-AI-TASK")},agents:{raw:agents,html:simpleMarkdownHtml(agents,"AGENTS")},prd:{raw:prd,html:simpleMarkdownHtml(prd,"PRD")}};progressMode="status";render()}
function bindProgress(){document.querySelectorAll("[data-progress-mode]").forEach(btn=>btn.onclick=()=>{progressMode=btn.dataset.progressMode;render()});const picker=document.getElementById("projectFolderInput");if(picker)picker.onchange=e=>loadProjectFolder(e.target.files)}

...
def quota_guardian_status() -> dict:
    ...
    launcher = ROOT / ("開啟配額守門員.bat" if is_windows else "開啟配額守門員.command")
    # Windows 沒有 swift 工具鏈，浮動視窗走 quota_guard_floating.py（Tkinter）；
    # macOS 仍走 quota_guard_floating.swift。檢查存在性時要對齊各自平台實際在用的檔案。
    floating_app = ROOT / "scripts" / ("quota_guard_floating.py" if is_windows else "quota_guard_floating.swift")
    snapshot_file = ROOT / "scripts" / "quota_guard_snapshot.py"
    claude_bridge = ROOT / "scripts" / ("claude_quota_statusline.ps1" if is_windows else "claude_quota_statusline.sh")
    ...
    files_ready = all(path.exists() for path in [launcher, floating_app, snapshot_file, claude_bridge, bridge_installer])
    automation_ready = automation_file.exists()
    statusline_ready = False
    bridge_wrapper = Path.home() / ".claude" / ("cctokmon-bridge.ps1" if is_windows else "cctokmon-bridge.sh")
    ...
    install_prompt = "\n".join(
        [
            "請幫我在目前這台電腦把「Codex × Claude Code 配額守門員」安裝好，並直接完成可用版本。",
            "",
            "專案路徑：",
            str(ROOT),
            "",
...
<title>新手 AI 外掛控制台｜Codex × Claude Code 入門系統</title>
<style>
  ...
  *{box-sizing:border-box; margin:0; padding:0;}
  ...
  .tab{padding:8px 14px; border-radius:10px 10px 0 0; background:transparent; border:none; color:var(--muted); font-size:.92rem; cursor:pointer; border-bottom:2px solid transparent; text-decoration:none; display:inline-flex; align-items:center;}
  ...
  .checkbox-item{display:inline-flex; align-items:center; gap:6px; background:var(--panel); border:1px solid var(--border); border-radius:8px; padding:6px 10px; font-size:.84rem; cursor:pointer;}
...
<body>
<header><div class="head-inner"><h1>新手 AI 外掛控制台 <span class="sub">Codex × Claude Code 入門系統</span></h1><div class="tabs"><button class="tab active" data-tab="guide">新手快速開始</button><button class="tab" data-tab="installGuide">安裝與外掛</button><button class="tab" data-tab="customSkill">個人化設定</button><button class="tab" data-tab="loopPrompt">Loop 提示詞</button><button class="tab" data-tab="automation">自動化設定</button><button class="tab" data-tab="daily">日常提示詞</button><button class="tab" data-tab="prompts">提示詞庫</button></div></div></header>
...
const PAGE_INTROS = {
  installGuide:{title:"安裝與外掛",lead:"這頁專門介紹怎麼下載官方 Codex、Claude Code，最後再說明這個公開頁面只負責提示詞瀏覽與複製。",purpose:"把官方工具的下載與登入流程一次講清楚，再說明這個網站本身不會替你執行任何安裝動作。",first:"先決定你要用 Codex 還是 Claude Code，再從這頁的官方連結下載對應版本。",when:"第一次接觸 Codex / Claude Code，想知道從哪裡下載官方版本時。"},
  prompts:{title:"提示詞庫",lead:"常用提示詞集中管理，適合你直接複製給 Codex 或 Claude Code（VS Code）。",purpose:"快速啟動工作流程。",first:"先選分類，再從卡片中選想要的提示詞複製。",when:"要交辦任務、請 AI 審查、整理交接或產出文件時。"},
  ...
  customSkill:{title:"個人化設定",lead:"用選單自動組出「個人化專屬 skill」提示詞。使用者先選職業、用途、AI 風格、安裝目標，再複製整段給 Codex 或 Claude Code 使用。",purpose:"把你的工作痛點與陪練方式整理成可重複使用的提示詞模板。",first:"進頁面後會跳出選單；先選好欄位，再直接複製右側的提示詞。",when:"想讓 AI 先理解你的職務與業務，再替你做挑刺陪練時。"},
  automation:{title:"自動化設定",lead:"用表單組出「請 AI 幫我把這個流程做成自動化」的提示詞，整段複製即可。",purpose:"把自動化需求整理成提示詞模板，不在這頁觸發任何本機動作。",first:"先填問題、痛點、預期輸出，右側會即時生成可複製的提示詞。",when:"想把 AI 工作流程做成固定步驟、整理自動化操作入口時。"},
  guide:{title:"新手快速開始",lead:"這一頁是新手版入口，專門幫你用最簡單的方式認識這套提示詞控制台，知道先看哪邊、怎麼複製、怎麼用。",purpose:"讓第一次接觸的人不用先懂很多指令，也能照著頁面把官方工具與提示詞順順接起來。",first:"先看下方的新手 3 步驟，再按你現在需要的入口進去。",when:"第一次使用這個頁面、要教新手上手，或想快速整理自己目前該從哪裡開始時。"},
  loopPrompt:{title:"Loop 提示詞",lead:"先用白話文搞懂「Loop Engineering」是什麼，再用選單組出一份完整的 Loop 提示詞。產生的提示詞會要求 Codex／Claude Code 用選擇題追問你還沒填清楚的地方，最後才會給你一份能直接拿去跑的 Loop 設定。",purpose:"把 Loop Contract 六要素（Trigger／Scope／Action／Budget／Stop Condition／Report）整理成一段可直接交辦的提示詞。",first:"先用一句話講你現在卡在哪裡，不確定的欄位都可以先選「不確定，請用選擇題幫我決定」。",when:"想讓 AI 自動重複跑一套流程（修 Bug、寫測試、整理文件、同步備份等），而不是每次都重新打字下指令時。"}
};
...
function renderOnlineBanner(){const el=document.getElementById("onlineBanner");if(!el)return;const isOnline=/\.github\.io$/i.test(location.hostname);if(!isOnline){el.innerHTML="";return}el.innerHTML=`<div class="online-banner"><div class="grow"><b>這是公開展示版，只提供提示詞瀏覽與複製</b><div class="summary">頁面上的每張卡片都是純文字提示詞模板。你可以直接挑分類、看內容、按下複製，再貼給 Codex 或 Claude Code 使用。不會在這個網頁上執行任何下載、安裝或還原動作。</div><div class="summary" style="margin-top:6px">如果你想把整套系統下載到自己的電腦慢慢客製（例如改 <code>data/prompts.yaml</code>、<code>data/skills.yaml</code>、做自己的版本），下面這顆按鈕會帶你到「安裝與外掛」頁查看 Codex / Claude Code 官方下載教學。</div></div><button type="button" data-tab-jump="installGuide">下載到自己電腦</button></div>`;el.querySelector("[data-tab-jump]")?.addEventListener("click",()=>setTab("installGuide"))}
...
function homeHtml(){return`<div class="wide-sop"><div class="home-hero"><h2>新手 AI 提示詞控制台</h2><p>這個公開頁面只做一件事：把可以複製的提示詞整理在不同分類底下，方便你挑、看、複製，再貼給 Codex 或 Claude Code 使用。頁面上不會幫你下載、安裝或執行任何本機腳本。</p><div class="summary" style="margin-top:10px">如果你是第一次接觸 AI 開發，建議先從 <b>Codex Pro</b> 開始慢慢摸熟，習慣怎麼和 AI 對話、下任務、看它的回覆，再依需要加上 Claude Code。</div><div class="summary" style="margin-top:10px">這套提示詞是我累積約五年 AI 使用經驗、一邊實作一邊整理出來的。目的不是一次學會全部，而是讓你先用得起來，再慢慢理解每張卡片在幫什麼。</div><div class="home-actions" style="margin-top:14px"><button class="copy-btn" type="button" data-home-tab="installGuide">先看安裝教學</button><button class="copy-btn" type="button" data-home-tab="prompts">我要複製提示詞</button><button class="copy-btn" type="button" data-home-tab="daily">我只想先試一段日常提示詞</button><button class="copy-btn" type="button" data-home-tab="loopPrompt">建立 Loop 提示詞</button></div><div class="home-kpis"><div class="home-kpi"><b>新手先做什麼</b><div class="summary">先把 Codex 或 Claude Code 裝好，再回到這頁找你現在要用的提示詞。</div></div><div class="home-kpi"><b>這個頁面在幫什麼</b><div class="summary">把可重複使用的提示詞按分類整理好，讓你不用每次自己重寫。</div></div><div class="home-kpi"><b>使用原則</b><div class="summary">先用會用到的，不用第一天就把全部分類都研究完。</div></div></div></div><div class="home-panel" style="margin:16px 0"><h3>新手最簡單的使用順序</h3><div class="summary">第 1 步看「安裝與外掛」把官方 Codex / Claude Code 準備好 → 第 2 步到「日常提示詞」或「提示詞庫」找現在要用的內容 → 第 3 步把卡片複製貼給 AI，就能開始工作。</div><div class="summary" style="margin-top:8px">想要循環式、要 AI 反覆跑同一類任務時，可以跳到「Loop 提示詞」分類，用 Loop Contract 6 要素組出可直接交辦的提示詞。</div></div><div class="home-steps"><div class="home-step"><span class="num">1</span><h3>先把工具準備好</h3><div class="summary">到「安裝與外掛」看官方 Codex / Claude Code 怎麼下載、登入。這頁本身不會替你安裝任何東西。</div></div><div class="home-step"><span class="num">2</span><h3>再找現在要用的提示詞</h3><div class="summary">想直接交辦事情，就去「提示詞庫」；想先試一段最簡單的問法，就到「日常提示詞」；想設計可自動重複跑的流程，就去「Loop 提示詞」。</div></div><div class="home-step"><span class="num">3</span><h3>最後複製、貼給 AI</h3><div class="summary">每張卡片都附複製按鈕，按下後直接貼到 Codex 或 Claude Code 對話框即可。</div></div></div><div class="home-stack"><div class="home-panel"><h3>這個頁面在幫什麼</h3><div class="mini-list"><div class="mini-item"><b>找提示詞更快</b><div class="summary">分類清楚、可搜尋，不用每次都重寫一遍提示詞。</div></div><div class="mini-item"><b>提示詞直接複製</b><div class="summary">不知道怎麼下指令時，直接複製整理好的版本交給 AI 即可。</div></div><div class="mini-item"><b>純瀏覽、純複製</b><div class="summary">這頁不會在你的電腦上跑任何安裝、還原或啟動腳本。</div></div></div></div><div class="home-panel"><h3>每個分類在做什麼</h3><div class="mini-list"><div class="mini-item"><b>安裝與外掛</b><div class="summary">教你怎麼下載官方 Codex / Claude Code，適合第一次接觸的人。</div></div><div class="mini-item"><b>個人化設定</b><div class="summary">用選單組出個人化「給 AI 的提示詞模板」，整段複製給 AI 使用。</div></div><div class="mini-item"><b>Loop 提示詞</b><div class="summary">用 Loop Contract 六要素組出循環式提示詞，讓 AI 自動一輪一輪幫你跑。</div></div><div class="mini-item"><b>自動化設定</b><div class="summary">用表單組出「請 AI 幫我把這個流程做成自動化」的提示詞。</div></div><div class="mini-item"><b>日常提示詞</b><div class="summary">整理最常用的 AI 問法，讓你不知道怎麼開口時可以直接複製。</div></div><div class="mini-item"><b>提示詞庫</b><div class="summary">把較完整的工作提示詞整理在一起，適合正式交辦任務時用。</div></div></div></div></div></div>`}
function openQuotaGuardian(){}
function wakeAI(){}
...
function dailyQuestionHtml(question){const answer=dailyAnswerEntry(question.id);const inputAttrs=question.type==="textarea"?`placeholder="${esc(question.placeholder||"可留空，之後再補")}"`:`type="text" value="${esc(answer.text)}" placeholder="${esc(question.placeholder||"可留空，之後再補")}"`;const inputBody=question.type==="textarea"?`<textarea class="daily-modal-textarea" data-daily-text="${esc(question.id)}" placeholder="${esc(question.placeholder||"可留空，之後再補")}">${esc(answer.text)}</textarea>`:`<input class="daily-modal-input" data-daily-text="${esc(question.id)}" ${inputAttrs}>`;return`<div class="daily-modal-question"><div class="daily-modal-step">逐題整理需求</div><h4>${esc(question.label)}</h4>${question.type==="choice"?`<div class="daily-choice-grid">${(question.options||[]).map(option=>dailyChoiceButton(question,option)).join("")}</div>`:""}${inputBody}</div>`}
function dailyBuiltPrompt(card){const questionLines=dailyQuestionList(card).map(question=>`- ${question.label}：${dailyAnswerLabel(question)}`).join("\n");return`請先根據下面這些回答整理需求，若資訊不足，先用最少問題補問，再執行後面的原始提示詞。\n\n卡片：${card.title}\n適用情境：${card.when}\n\n使用者回答：\n${questionLines}\n\n原始提示詞：\n${card.prompt}`}
function dailyWizardHtml(){if(!dailyWizardState.cardId)return"";const meta=dailyFindCard(dailyWizardState.cardId);if(!meta)return"";const {card}=meta;const questions=dailyQuestionList(card);const previewStep=questions.length;const step=Math.min(dailyWizardState.step,previewStep);if(step>=previewStep){const prompt=dailyBuiltPrompt(card);return`<div class="daily-modal" id="dailyWizard"><div class="daily-modal-backdrop" data-daily-close="1"></div><div class="daily-modal-card"><div class="daily-modal-head"><div><div class="daily-modal-step">完成提示詞生成</div><div class="daily-modal-title">${esc(card.title)}</div><div class="summary">${esc(card.when)}</div></div><button class="daily-modal-close" type="button" data-daily-close="1">關閉</button></div><div class="daily-modal-body"><div class="daily-modal-preview"><div class="summary"><b>這就是要交給 AI 的最終提示詞</b></div><pre class="prompt-body" id="dailyWizardPrompt">${esc(prompt)}</pre></div><div class="daily-modal-actions"><div class="group"><button class="copy-btn secondary-btn" type="button" data-daily-back="1">回上一題</button></div><div class="group"><button class="copy-btn" type="button" id="dailyWizardCopy" data-copy="${encodeURIComponent(prompt)}">複製生成後提示詞</button></div></div></div></div></div>`}const question=questions[step];return`<div class="daily-modal" id="dailyWizard"><div class="daily-modal-backdrop" data-daily-close="1"></div><div class="daily-modal-card"><div class="daily-modal-head"><div><div class="daily-modal-step">第 ${step+1} 題 / 共 ${questions.length} 題</div><div class="daily-modal-title">${esc(card.title)}</div><div class="summary">${esc(card.when)}</div></div><button class="daily-modal-close" type="button" data-daily-close="1">關閉</button></div><div class="daily-modal-body">${dailyQuestionHtml(question)}<div class="daily-modal-actions"><div class="group">${step>0?`<button class="copy-btn secondary-btn" type="button" data-daily-back="1">上一題</button>`:""}<button class="copy-btn secondary-btn" type="button" data-daily-close="1">先關閉</button></div><div class="group"><button class="copy-btn" type="button" data-daily-next="1">${step===questions.length-1?"完成並產生提示詞":"下一題"}</button></div></div></div></div></div>`}
function dailyPromptCard(card,sectionIndex,cardIndex){const cardId=dailyCardId(sectionIndex,cardIndex);return`<div class="daily-card"><h4>${esc(card.title)}</h4><div class="usage">${esc(card.when)}</div><pre class="prompt-body">${esc(card.prompt)}</pre><div class="daily-card-actions"><button class="copy-btn" type="button" data-daily-open="${esc(cardId)}">開始互動式提示詞生成</button></div></div>`}
...
function installWakeCard(){return""}
...
function bundledSkillsInstallHtml(){
  const REPO_RAW="https://raw.githubusercontent.com/kagenhsu/codex-claude-skills-backup/main";
  const installAllMac=`curl -fsSL ${REPO_RAW}/scripts/install-all-skills.sh | bash`;
  const installAllWin=`iwr ${REPO_RAW}/scripts/install-all-skills.ps1 -OutFile $env:TEMP\\install-all-skills.ps1; & $env:TEMP\\install-all-skills.ps1`;
  const bundled=[
    {name:"dual-ai-workflow",badge:"自製 skill",summary:"二刀流工作流：Codex 當主力工程師（規劃／實作／測試／修正／git 存檔），Claude Code（VS Code）當審查員（架構審查／程式碼審查／P0-P2 風險分級／複審）。整段流程靠 DUAL-AI-STATE.md 在兩支 AI 之間同步進度，斷線重接也不會忘記做到哪。",when:"想跨 AI 接力寫程式、想換電腦／換 AI 也不會忘記做到哪。"},
    {name:"plan-progress",badge:"自製 skill",summary:"讀專案根目錄的 專案規劃表.md，自動對照資料夾內檔案，30 秒內告訴你目前做到哪個階段／哪個小步驟、下一步該幹嘛、完成度幾%。第一次跑會把推論結果寫進 .plan-rules.yaml 供下次重用。",when:"接手放置一段時間的舊專案、雙刀流交棒前、想用一句話回答「下一步該幹嘛」。"}
  ];
  const cards=bundled.map(s=>{
    const macCmd=`curl -fsSL ${REPO_RAW}/scripts/install-skill.sh | bash -s ${s.name}`;
    const winCmd=`iwr ${REPO_RAW}/scripts/install-skill.ps1 -OutFile $env:TEMP\\install-skill.ps1; & $env:TEMP\\install-skill.ps1 -Skill ${s.name}`;
    return `<div class="install-card"><h3>${esc(s.name)} <span class="cat-tag">${esc(s.badge)}</span></h3><div class="install-subtitle">${esc(s.when)}</div><div class="summary">${esc(s.summary)}</div>${cmdRow("macOS / Linux（複製後貼到終端機）",macCmd)}${cmdRow("Windows（複製後貼到 PowerShell）",winCmd)}<div class="summary" style="font-size:.78rem;color:var(--muted)">會同時裝到 <code>~/.codex/skills/${esc(s.name)}</code> 與 <code>~/.claude/skills/${esc(s.name)}</code>。若目錄已存在會自動加 <code>.bak.YYYYMMDD</code> 後綴備份，不會直接覆蓋。安裝完請重啟 Codex / Claude Code。</div></div>`;
  }).join("");
  return `<div class="card install-hero" style="margin-top:14px"><h2>📦 一鍵安裝這個 repo 內附的 skills</h2><div class="summary">這裡直接給你「一行安裝全部」的指令。按下複製後，貼到終端機或 PowerShell 按 Enter，就會把這個 repo 備份包內實際存在的 skill 一次裝到 <code>~/.codex/skills</code> 與 <code>~/.claude/skills</code>。</div>${cmdRow("macOS / Linux：一行安裝這個 repo 內全部 skills",installAllMac)}${cmdRow("Windows：一行安裝這個 repo 內全部 skills",installAllWin)}<div class="privacy-note">重要：目前這個 repo 備份包內實際只有 <code>dual-ai-workflow</code> 與 <code>plan-progress</code> 兩個真實 skill。網站上其他 baoyu-* 卡片是說明資料，不在這個 repo 備份包裡，不能用這裡一鍵安裝。</div></div><div class="install-grid">${cards}</div>`;
}
function installGuideHtml(){return`<div class="sop wide-sop"><div class="card install-hero"><h2>Codex / Claude Code 安裝與設定教學</h2><div class="summary">這頁專門給第一次接觸的人看。先把官方 Codex 或 Claude Code 裝好、登入好，再回到這個頁面找你需要的提示詞複製使用。</div><div class="privacy-note">這一頁只放官方下載連結與教學說明，以及可以「複製到自己終端機執行」的安裝指令。本網站不會在你的瀏覽器裡偷偷執行任何本機腳本 — 真正的安裝動作要等你貼上指令、按下 Enter 才會跑。如果你只是想要原始檔自己改，下面卡片 #3 仍有 GitHub repo 連結；如果你是要直接安裝，這頁下面已補一行安裝全部內附 skill 的按鈕。</div></div><div class="install-grid"><div class="install-card featured"><h3>1. 安裝 Codex</h3><div class="install-subtitle">建議新手先從桌面版開始，再決定要不要補 IDE extension</div><div class="install-steps"><div class="install-step"><span>1</span><div>到官方頁下載 Codex app，安裝後打開。</div></div><div class="install-step"><span>2</span><div>用 ChatGPT 帳號登入，或依需求改用 OpenAI API key。</div></div><div class="install-step"><span>3</span><div>選一個專案資料夾，確認是本機模式，再開始第一個任務。</div></div></div><div class="summary">如果你主要在 VS Code、Cursor 或 Windsurf 工作，也可以再安裝 Codex extension，讓它直接出現在編輯器側欄。</div><div class="summary" style="margin-top:6px">如果你是 Intel Mac，請改用官方安裝頁選 Intel 版本。</div><div class="home-panel-actions"><a class="copy-btn" href="https://persistent.oaistatic.com/codex-app-prod/Codex.dmg" target="_blank" rel="noreferrer">下載最新 macOS 版</a><a class="copy-btn" href="https://get.microsoft.com/installer/download/9PLM9XGG6VKS?cid=website_cta_psi" target="_blank" rel="noreferrer">下載最新 Windows 版</a><button class="copy-btn tutorial-btn" type="button" data-install-guide-topic="codex">查看教學</button></div></div><div class="install-card featured"><h3>2. 安裝 Claude Code</h3><div class="install-subtitle">如果你習慣用 VS Code 審查、補改與複審，這一套很適合當第二支 AI</div><div class="summary">這裡走官方最新版下載入口：先裝 Claude，再補 VS Code，就可以在編輯器內使用。</div><div class="home-panel-actions"><a class="copy-btn" href="https://claude.com/download" target="_blank" rel="noreferrer">下載最新 macOS 版</a><a class="copy-btn" href="https://claude.ai/api/desktop/win32/x64/exe/latest/redirect" target="_blank" rel="noreferrer">下載最新 Windows 版</a></div><div class="home-panel-actions"><a class="copy-btn tutorial-btn" href="https://code.visualstudio.com/sha/download?build=stable&os=darwin-universal" target="_blank" rel="noreferrer">下載 VS Code macOS 版</a><a class="copy-btn tutorial-btn" href="https://code.visualstudio.com/sha/download?build=stable&os=win32-x64-user" target="_blank" rel="noreferrer">下載 VS Code Windows 版</a></div><div class="home-panel-actions"><button class="copy-btn tutorial-btn" type="button" data-install-guide-topic="claude">查看教學</button></div><div class="summary">VS Code 裝好後，再照官方教學登入 Claude Code extension 即可。</div></div><div class="install-card"><h3>3. 想直接裝到自己電腦？</h3><div class="install-subtitle">你要的是一行安裝全部內附 skill，不是只看 GitHub 原始檔</div><div class="summary">如果你要的是「一行複製後貼到終端機，就把這個 repo 內全部可安裝的 skill 裝進自己電腦」，請直接看下方「📦 一鍵安裝這個 repo 內附的 skills」區塊。那裡已經補成一行安裝全部的按鈕。</div><div class="summary">如果你之後還想自己改 <code>data/prompts.yaml</code>、<code>data/skills.yaml</code> 或重做頁面，再用下面 GitHub 連結去看原始檔。</div><div class="home-panel-actions"><a class="copy-btn" href="https://github.com/kagenhsu/codex-claude-skills-backup" target="_blank" rel="noreferrer">到 GitHub repo 看原始檔</a></div></div></div>${bundledSkillsInstallHtml()}${installGuideTutorialHtml()}</div>`}
...
function bindCustomSkill(){document.querySelectorAll("[data-custom-skill-input]").forEach(el=>el.oninput=el.onchange=updateCustomSkill);const copy=document.getElementById("customSkillCopy");if(copy)copy.onclick=()=>copyText(document.getElementById("customSkillPrompt")?.textContent||"",copy);const reset=document.getElementById("customSkillReset");if(reset)reset.onclick=()=>{localStorage.removeItem("custom-skill-builder");const contentEl=document.getElementById("content");if(contentEl){contentEl.innerHTML=customSkillHtml();bindCustomSkill()}}}
const LOOP_UNSURE="不確定，請用選擇題幫我決定";
const LOOP_TASK_TYPES=["自動修 Bug","自動寫測試","Code Review 把關","文件 / Changelog 自動更新","資料同步或備份","開發系統跟網頁","自訂任務"];
const LOOP_TOOLS=["Codex 專用","Claude Code 專用","Codex + Claude Code 都用（Maker-Checker）"];
const LOOP_TRIGGERS=["手動執行（我自己按下開始）","排程時間（例如每天/每小時自動跑）","Git 事件（commit / PR / CI 失敗時觸發）",LOOP_UNSURE];
const LOOP_BUDGETS=["輪數上限（例如最多跑 20 輪就停）","金額上限（例如最多花 5 美金）","Token 上限",LOOP_UNSURE];
const LOOP_STOP_CONDITIONS=["測試全部通過才算完成","連續 N 次失敗就強制停止","需要我人工確認才算完成",LOOP_UNSURE];
const LOOP_REPORTS=["在終端機印出結果就好","寫進 Git commit 訊息","發通知到 Slack",LOOP_UNSURE];
const LOOP_MAKER_CHECKER=["先用最簡單的 Ralph Loop 就好（不用雙重把關）","我要 Maker-Checker 雙重確認（多一個 AI 專門檢查）","不確定，請 AI 幫我評估要不要用"];
const LOOP_PROMPT_DEFAULTS={painPoint:"",taskType:["自動修 Bug"],tool:"Codex 專用",trigger:LOOP_UNSURE,scope:"",budget:LOOP_UNSURE,stopCondition:LOOP_UNSURE,report:LOOP_UNSURE,makerChecker:"先用最簡單的 Ralph Loop 就好（不用雙重把關）",extra:"請用繁體中文。遇到要刪檔案、改動主分支、或花費超出預算，一定要先停下來問我。"};
function loopPromptData(){const stored=JSON.parse(localStorage.getItem("loop-prompt-builder")||"{}");const merged={...LOOP_PROMPT_DEFAULTS,...stored};const storedTaskTypes=Array.isArray(merged.taskType)?merged.taskType:String(merged.taskType||"").split(",").map(s=>s.trim()).filter(Boolean);merged.taskType=storedTaskTypes.length?storedTaskTypes:LOOP_PROMPT_DEFAULTS.taskType;return merged}
function loopPromptRead(){const checkedTaskTypes=Array.from(document.querySelectorAll("[data-loop-task-type]:checked")).map(cb=>cb.value);const data={painPoint:formVal("loopPainPoint"),taskType:checkedTaskTypes.length?checkedTaskTypes:LOOP_PROMPT_DEFAULTS.taskType,tool:formVal("loopTool"),trigger:formVal("loopTrigger"),scope:formVal("loopScope"),budget:formVal("loopBudget"),stopCondition:formVal("loopStopCondition"),report:formVal("loopReport"),makerChecker:formVal("loopMakerChecker"),extra:formVal("loopExtra")};localStorage.setItem("loop-prompt-builder",JSON.stringify(data));return data}
function loopAskLine(label,value,hint){return value===LOOP_UNSURE?`- ${label}：還沒決定，請用選擇題（附 A/B/C/D 選項）幫我問清楚再繼續。${hint?"提示："+hint:""}`:`- ${label}：${value}`}
function loopPromptText(d){const needsScope=!d.scope.trim();const needsPainPoint=!d.painPoint.trim();const needsAsk=[d.trigger,d.budget,d.stopCondition,d.report,d.makerChecker].includes(LOOP_UNSURE)||needsScope||needsPainPoint;return`我剛學完 Loop Engineering 的概念：Prompt 是單次指令、Harness 是給 AI 的裝備（記憶檔案、工具、判官、預算、剎車）、Loop 是會看狀況做決策的控制流程。一個安全的 Loop 要填滿 6 個欄位的 Loop Contract：Trigger、Scope、Action、Budget、Stop Condition、Report。

請你用 ${d.tool} 的方式，協助我把下面的 Loop Contract 填完整，並產出一個我能直接拿去跑的 Loop。

我目前遇到的痛點：${needsPainPoint?"我還沒描述清楚，請先用一個開放式問題簡單問我目前最困擾、最重複的工作是什麼，再繼續往下確認六要素。":d.painPoint}

我目前已經決定的設定：
- 任務（Action 的基礎，可能不只一種）：${d.taskType.join("、")}${d.taskType.length>1?"\n  （選了多種任務，請先幫我確認這些任務要合併成同一個 Loop 一起跑，還是分成多個各自獨立的 Loop，再繼續往下走。）":""}
${loopAskLine("Trigger 觸發方式",d.trigger,"手動執行 / 排程時間 / Git 事件（commit、PR、CI 失敗）")}
${needsScope?`- Scope 作用範圍：還沒填，請用選擇題幫我確認可以動哪個分支、哪些檔案、不能碰什麼。`:`- Scope 作用範圍：${d.scope}`}
${loopAskLine("Budget 預算上限",d.budget,"輪數上限 / 金額上限 / Token 上限")}
${loopAskLine("Stop Condition 停止條件",d.stopCondition,"測試全過 / 連續 N 次失敗就停 / 需要我人工確認")}
${loopAskLine("Report 回報方式",d.report,"終端機輸出 / Git commit 訊息 / Slack 通知")}
${loopAskLine("是否需要 Maker-Checker 雙重把關",d.makerChecker,"先用簡單 Ralph Loop / 要雙重確認")}
- 其他要求：${d.extra||"請用繁體中文，遇到不確定的地方先問我。"}

第一段：用選擇題把六要素問到完整
${needsAsk?`上面標「還沒決定」或「還沒填」的欄位，請你一次最多問 5 題，每題都用選擇題格式（A/B/C/D，必要時附「其他，我自己說」），不要問開放式問題。問到 Trigger、Scope、Action、Budget、Stop Condition、Report 六個欄位都確定為止，再進入第二段。`:`六個欄位目前都已經填好。請先用一段話跟我確認你的理解是否正確，如果有任何矛盾或風險（例如預算和停止條件衝突），用選擇題問我要怎麼調整，確認沒問題才進入第二段。`}

第二段：產出 Harness 三件套
等六要素都確定後，幫我產出以下三個檔案的完整內容：
1. prompt.md：這個 Loop 每一輪要執行的具體指令（Action），要包含明確的成功判定方式
2. vision.md：這個 Loop 存在的目的與不能做的事（對應 Scope 的邊界）
3. progress.txt：用來記錄目前跑到第幾輪、上次結果，讓 Loop 有「記憶」

第三段：產出可執行的 Loop 腳本
幫我寫一個最小可行的 Ralph Loop（不用一次做到複雜），但必須包含三道剎車：
1. 輪數上限（對應 Budget）
2. 連續失敗就強停（例如連續 3 輪同一個錯誤、或連續 5 輪沒有任何 diff 就停）
3. 達成 Stop Condition 就正常結束，並依 Report 的方式回報結果

${d.makerChecker.includes("雙重確認")?`第四段：Maker-Checker 雙重把關\n因為我選了雙重把關，請額外幫我寫一份獨立的 Checker 提示詞：\n1. Checker 必須在「全新對話」中執行，只能看 spec 和 diff，不能看 Maker 的推理過程\n2. Checker 只回答「通過」或「不通過 + 具體原因」，不能改程式碼\n3. 不通過時，把原因整理好退回給 Maker 重做`:`第四段：之後要不要升級\n先不用 Maker-Checker。但完成後，請簡單跟我說明如果之後想加雙重把關，需要多準備什麼。`}

安全規則（一定要遵守）：
1. 不要刪除任何既有檔案或資料夾，不要用批量刪除指令
2. 不要動 Scope 範圍以外的分支或檔案，尤其不能直接改 main
3. 預算用完、或連續出錯達到剎車條件，一定要停下來等我確認，不能自己延長
4. 每一輪做了什麼、為什麼停，都要照 Report 的方式讓我看得到

請先從第一段開始，不要跳過六要素確認，也不要還沒確認就直接動手寫檔案或執行指令。`}
function loopPromptHtml(){const d=loopPromptData();return`<div class="sop wide-sop"><div class="card"><h2>新手先讀：Loop Engineering 是什麼？</h2><div class="summary">以前是「你跟 AI 一句一句聊」（寫 Prompt）；Loop Engineering 是「你設計一套會自動重複運作的流程，讓這套流程自己去跟 AI 對話、自己檢查、自己重複做」（寫 Loop）。可以想成你從「自己炒菜的廚師」升級成「設計菜單和流程的餐廳老闆」。</div><div class="workflow-guide" style="margin-top:10px"><div class="guide-item"><div class="guide-label">Prompt（一句話）</div><div class="guide-text">單次、沒有記憶的指令。問完就結束，AI 不會記得上一輪。</div></div><div class="guide-item"><div class="guide-label">Harness（裝備）</div><div class="guide-text">幫 AI 配備「記憶檔案」「能操作 Git/Shell 的工具」「檢查對錯的判官」「預算上限」「連續出錯就強停的剎車」。</div></div><div class="guide-item"><div class="guide-label">Loop（決策者）</div><div class="guide-text">不是死板重複，而是看狀況決定要不要繼續、要不要換做法、什麼時候該停。</div></div></div><h3 style="margin-top:12px">一個安全的 Loop 要回答 6 個問題（Loop Contract）</h3><div class="workflow-guide" style="margin-top:10px"><div class="guide-item"><div class="guide-label">Trigger 觸發</div><div class="guide-text">什麼時候啟動？</div></div><div class="guide-item"><div class="guide-label">Scope 範圍</div><div class="guide-text">只能動哪裡？</div></div><div class="guide-item"><div class="guide-label">Action 動作</div><div class="guide-text">每一輪要做什麼？</div></div><div class="guide-item"><div class="guide-label">Budget 預算</div><div class="guide-text">最多花多少（次數／金額／Token）？</div></div><div class="guide-item"><div class="guide-label">Stop Condition 停止條件</div><div class="guide-text">什麼時候算成功或失敗該停？</div></div><div class="guide-item"><div class="guide-label">Report 回報</div><div class="guide-text">結束後跟誰說、怎麼說？</div></div></div><div class="summary" style="margin-top:10px">下面的設定精靈就是在幫你把這 6 格填好。不確定的地方可以選「不確定，請用選擇題幫我決定」，產生的提示詞會要求 Codex／Claude Code 用選擇題（附選項）一步步問你，問到夠清楚為止，才會動手寫 Loop。</div></div><div class="home-split"><div class="card"><h2>設定你的 Loop</h2><div class="summary">不確定就選「不確定，請用選擇題幫我決定」，讓 AI 在對話中追問你。</div><div class="form-grid" style="margin-top:10px"><div class="field full"><label>痛點敘述（你現在卡在哪裡？）</label><textarea id="loopPainPoint" data-loop-prompt-input placeholder="例如：這個專案的 skills／設定常常改完忘記同步備份，每次都要手動檢查 index.html、skills 資料夾跟備份目錄是否一致，想要做一個自動偵測差異、自動同步備份的 Loop。">${esc(d.painPoint)}</textarea><div class="help">用自己的話講清楚現在重複在做、覺得麻煩或容易出錯的事，AI 會根據這段話幫你推敲 Action（每輪要做什麼）和 Stop Condition（什麼時候算做完）。</div></div><div class="field full"><label>這個 Loop 要自動做什麼任務（可多選）</label><div class="checkbox-group" id="loopTaskTypeGroup">${LOOP_TASK_TYPES.map(t=>`<label class="checkbox-item"><input type="checkbox" value="${esc(t)}" data-loop-task-type ${d.taskType.includes(t)?"checked":""}>${esc(t)}</label>`).join("")}</div></div><div class="field"><label>使用哪個 AI CLI</label><select id="loopTool" data-loop-prompt-input>${optionsHtml(LOOP_TOOLS,d.tool)}</select></div><div class="field"><label>Trigger 觸發方式</label><select id="loopTrigger" data-loop-prompt-input>${optionsHtml(LOOP_TRIGGERS,d.trigger)}</select></div><div class="field"><label>Budget 預算上限</label><select id="loopBudget" data-loop-prompt-input>${optionsHtml(LOOP_BUDGETS,d.budget)}</select></div><div class="field"><label>Stop Condition 停止條件</label><select id="loopStopCondition" data-loop-prompt-input>${optionsHtml(LOOP_STOP_CONDITIONS,d.stopCondition)}</select></div><div class="field"><label>Report 回報方式</label><select id="loopReport" data-loop-prompt-input>${optionsHtml(LOOP_REPORTS,d.report)}</select></div><div class="field"><label>要不要 Maker-Checker 雙重把關</label><select id="loopMakerChecker" data-loop-prompt-input>${optionsHtml(LOOP_MAKER_CHECKER,d.makerChecker)}</select></div><div class="field full"><label>Scope 作用範圍</label><input id="loopScope" data-loop-prompt-input value="${esc(d.scope)}" placeholder="例如：只能改 feature/loop-test 分支，不能動 main，只能碰 src/ 底下檔案"><div class="help">沒填的話，提示詞會要求 AI 先用選擇題跟你確認範圍。</div></div><div class="field full"><label>其他要求</label><textarea id="loopExtra" data-loop-prompt-input placeholder="例如：請用繁體中文、遇到要刪檔案或動主分支一定要先停下來問我。">${esc(d.extra)}</textarea></div></div><div class="home-panel-actions" style="margin-top:10px"><button class="copy-btn tutorial-btn" type="button" id="loopPromptReset">清空重選</button></div></div><div class="card builder-preview-card"><h2>可複製提示詞</h2><pre class="prompt-body builder-preview" id="loopPromptOutput">${esc(loopPromptText(d))}</pre><div class="home-panel-actions"><button class="copy-btn" type="button" id="loopPromptCopy">複製提示詞</button></div></div></div></div>`}
function updateLoopPrompt(){const d=loopPromptRead();const out=document.getElementById("loopPromptOutput");if(out)out.textContent=loopPromptText(d)}
function bindLoopPrompt(){document.querySelectorAll("[data-loop-prompt-input]").forEach(el=>el.oninput=el.onchange=updateLoopPrompt);document.querySelectorAll("[data-loop-task-type]").forEach(cb=>cb.onchange=updateLoopPrompt);const copy=document.getElementById("loopPromptCopy");if(copy)copy.onclick=()=>copyText(document.getElementById("loopPromptOutput")?.textContent||"",copy);const reset=document.getElementById("loopPromptReset");if(reset)reset.onclick=()=>{localStorage.removeItem("loop-prompt-builder");const contentEl=document.getElementById("content");if(contentEl){contentEl.innerHTML=loopPromptHtml();bindLoopPrompt()}}}
...
function render(){const chips=document.getElementById("chips"),content=document.getElementById("content"),countLine=document.getElementById("countLine"),search=document.querySelector(".search"),searchSlot=document.getElementById("searchSlot");if(tab==="prompts")flowMode="common";renderOnlineBanner();renderLaunchTip();renderIntro();const showSearch=tab==="prompts";if(search){if(showSearch&&searchSlot){search.style.display="flex";searchSlot.appendChild(search)}else{search.style.display="none"}}chips.innerHTML=cats().map(c=>`<button class="chip ${c===cat?"active":""}" data-cat="${esc(c)}">${esc(c)}</button>`).join("");chips.querySelectorAll(".chip").forEach(ch=>ch.onclick=()=>{cat=ch.dataset.cat;if(tab==="prompts"&&cat!=="全部")flowMode="common";render()});if(tab==="installGuide"){countLine.textContent="";content.innerHTML=installGuideHtml();bindInstallGuide();return}if(tab==="guide"){countLine.textContent="";content.innerHTML=homeHtml();bindHome();return}if(tab==="daily"){countLine.textContent="";content.innerHTML=dailyHtml();bindDaily();return}if(tab==="customSkill"){countLine.textContent="";content.innerHTML=customSkillHtml();bindCustomSkill();return}if(tab==="loopPrompt"){countLine.textContent="";content.innerHTML=loopPromptHtml();bindLoopPrompt();return}if(tab==="automation"){countLine.textContent="";content.innerHTML=automationHtml();bindAutomation();return}if(tab==="prompts"){const commonPrompts=DATA.prompts.filter(p=>(p.flow||"common")==="common");const count=commonPrompts.filter(p=>(cat==="全部"||p.category===cat)&&match(p,["title","usage","category","prompt","flow","stage"])).length;countLine.textContent=`目前顯示 ${count} 則提示詞，總共 ${commonPrompts.length} 則`;content.innerHTML=renderPromptFlow();bindCapture()}}
...
document.querySelectorAll(".tab").forEach(t=>{if(t.dataset.tab)t.onclick=()=>setTab(t.dataset.tab)});document.getElementById("searchBox").addEventListener("input",e=>{q=e.target.value.trim();render()});const initialTab=(location.hash||"").slice(1);if(initialTab&&document.querySelector(`[data-tab="${initialTab}"]`)){setTab(initialTab)}else{render()}
</script>
</body>
</html>
'''
...
out = ROOT / "index.html"
# 公開頁面只保留提示詞瀏覽與複製，progress / backup / skills 三個 tab 已移除；
# 它們對應的大段 HTML 內容（含「配額守門員」、「備份 / 還原」、安裝腳本檔名等
# 字串）不再有任何 tab 引用，這裡一律注入空字串 / 空 JSON，避免它們以
# inline JS 字面值的形式殘留在 index.html 的 view source 中。
EMPTY_HTML_JSON = json.dumps("", ensure_ascii=False)
EMPTY_ARRAY_JSON = "[]"
html_out = (
    TEMPLATE.replace("__DATA_JSON__", data_json)
    .replace("__INSTALL_GUIDE_ASSETS__", install_guide_assets_json)
    .replace("__WORKFLOW_HTML__", EMPTY_HTML_JSON)
    .replace("__GUIDE_HTML__", EMPTY_HTML_JSON)
    .replace("__STATE_HTML__", EMPTY_HTML_JSON)
    .replace("__NEXT_HTML__", EMPTY_HTML_JSON)
    .replace("__AGENTS_HTML__", EMPTY_HTML_JSON)
    .replace("__PRD_HTML__", EMPTY_HTML_JSON)
    .replace("__PLAN_HTML__", EMPTY_HTML_JSON)
    .replace("__CONSOLE_FILE_PATHS__", EMPTY_ARRAY_JSON)
    .replace("__PROJECT_PATH__", json.dumps("", ensure_ascii=False))
    .replace("__PROJECT_URL__", json.dumps("", ensure_ascii=False))
